...
export function TasksPage() {
  ...
  const [statusSelections, setStatusSelections] = useState<Record<number, TaskStatus | ''>>({});
  ...
  async function handleStatusSelect(task: Task, nextStatus: TaskStatus | '') {
    if (!nextStatus || nextStatus === task.status) {
      setStatusSelections((current) => ({ ...current, [task.id]: '' }));
      return;
    }

    setStatusSelections((current) => ({ ...current, [task.id]: nextStatus }));
    ...
    <div className="space-y-6">
      ...
      {canCreate ? (
        <section className="border border-zinc-200 bg-white p-4">
          ...
          <form onSubmit={handleCreateTask} className="mt-4 grid gap-3 lg:grid-cols-[1fr_1fr_220px_auto]">
            ...
            <select value={assignedToId} onChange={(event) => setAssignedToId(event.target.value)} className={`h-10 rounded border border-zinc-200 px-3 text-sm outline-none focus:border-zinc-950 ${assignedToId ? 'text-zinc-900' : 'text-zinc-400'}`} required>
              <option value="">Select user</option>
              {users.length ? users.map((teamUser) => <option key={teamUser.id} value={teamUser.id}>{teamUser.name}</option>) : <option value="" disabled>No users available</option>}
            </select>
      ...
      <section className="border border-zinc-200 bg-white">
        <div className="grid gap-3 border-b border-zinc-200 p-4 md:grid-cols-[1fr_220px_auto]">
          ...
          <select value={status} onChange={(event) => setStatus(event.target.value as TaskStatus | '')} className={`h-10 rounded border border-zinc-200 px-3 text-sm outline-none focus:border-zinc-950 ${status ? 'text-zinc-900' : 'text-zinc-400'}`}>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[900px] w-full table-fixed divide-y divide-zinc-200 text-sm">
            <colgroup><col className="w-[34%]" /><col className="w-[16%]" /><col className="w-[18%]" /><col className="w-[18%]" /><col className="w-[14%]" /></colgroup>
            <thead className="bg-zinc-50 text-left text-xs uppercase text-zinc-500">
              <tr><th className="px-4 py-3">Task</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Assigned To</th><th className="px-4 py-3">Assigned By</th><th className="px-4 py-3">Update</th></tr>
            </thead>
            <tbody className="divide-y divide-zinc-100">
              {isLoading ? <TableLoading colSpan={5} label="Loading tasks" /> : null}
              {!isLoading && tasks.map((task) => (
                <tr key={task.id}>
                  ...
                  <td className="px-4 py-3"><select disabled={updatingTaskId === task.id} value={statusSelections[task.id] ?? ''} onChange={(event) => void handleStatusSelect(task, event.target.value as TaskStatus | '')} className={`h-9 w-full rounded border border-zinc-200 px-2 text-sm outline-none focus:border-zinc-950 disabled:cursor-not-allowed disabled:opacity-60 ${statusSelections[task.id] ? 'text-zinc-900' : 'text-zinc-400'}`}><option value="">Select status</option><option value="pending">Pending</option><option value="in_progress">In progress</option><option value="completed">Completed</option></select></td>
                </tr>
              ))}
              {!isLoading && !tasks.length ? <tr><td className="px-4 py-8 text-center text-zinc-500" colSpan={5}>No tasks found.</td></tr> : null}
            </tbody>
          </table>
        </div>
      </section>
